...
###
### 方法本质：这就是社会网络分析（SNA）的社区发现，但有三层方法论增量：
###   · 骨架 = SNA 社区发现：基因是节点，互为 top-k 的关系是边，
###     |rho| 是边权，cluster_louvain() 找模块度高的社区——把节点换成
###     "人"、边换成"互加好友"，代码一行不用改（Louvain 本就是 2008 年
###     为大规模社交网络设计的）。
###   · 增量一：建图用"互为 k 近邻图"（mutual k-NN，源自机器学习）。
###     SNA 的关系是现成的（谁认识谁），这里要从 18000×18000 相关矩阵
###     造出一张稀疏图。直接卡 |rho| 阈值会得到以必需基因为中心的
###     "名人图"——人人都连着它，整图糊成一团；mutual top-k（且关系）
###     强制留下互相选择的关系，图才散得开、模块才分得出。
###   · 增量二：关系带符号。正相关 = 共同必需，负相关 = 合成致死/互斥，
###     是两种生物学，分开建两张图分别聚类；负网络里的模块恰是
###     找靶点最有价值的。
###   · 增量三：跨尺度稳定性检验 = 把聚类结果的稳健性分析用成生物学
###     发现标准。Louvain 有随机性、对分辨率敏感是社区发现的著名缺陷；
###     "k 从小到大滑、找一直存在的模块"把"这是真功能单元还是算法
###     伪影"变成可计算判据（Jaccard ≥ 0.7 across k），同时充当
###     方法校验（已知复合体被捞回）和发现过滤器（稳定但未知的模块
###     才值得做实验）。
###   一句话：骨架是 SNA 社区发现，灵魂是 mutual rank 建图 + 跨尺度
###   稳定性；这个组合用在 DepMap 共依赖数据上，就是 CoffeeTime。
###
